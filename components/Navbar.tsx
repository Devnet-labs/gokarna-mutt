...
import { Menu, X, Home, BookOpen, Crown, GitBranch, Network, Heart, Phone, PlayCircle } from "lucide-react";
...
export default function Navbar() {
  ...
  // Short labels for the desktop bar (Hindi/Kannada use the longer translations on mobile)
  ...
  const links = [
    { href: "/#about", key: "nav.about", icon: BookOpen },
    ...
    { href: "/#seva", key: "nav.seva", icon: Heart },
  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-colors duration-500 ${
        scrolled ? "bg-[var(--color-maroon-deep)] shadow-lg" : "bg-transparent"
      }`}
    >
      <nav className="max-w-[1500px] mx-auto flex items-center justify-between gap-4 px-4 sm:px-6 py-3">
        ...
        <Link href="/" className="flex items-center gap-3 group shrink-0">
          ...
          <div className="leading-tight hidden sm:block">
            <div
              className="font-[var(--font-cinzel)] text-[15px] lg:text-base tracking-[0.12em] font-bold whitespace-nowrap text-[var(--color-gold-light)] drop-shadow-[0_2px_6px_rgba(0,0,0,0.7)]"
            >
              PARTAGALI JEEVOTTAM MATH
            </div>
            <div
              className="text-[10px] uppercase tracking-[0.25em] mt-0.5 whitespace-nowrap text-[var(--color-gold-light)]/85 font-semibold drop-shadow-[0_2px_6px_rgba(0,0,0,0.7)]"
            >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="xl:hidden bg-[#3d1207]/95 backdrop-blur-xl border-t border-[var(--color-gold)]/25 overflow-hidden"
          >
                ...
                return (
                  ...
                  >
                    <Link
                      ...
                      className="flex items-center gap-3 py-3 px-3 rounded-md text-[var(--color-gold-light)] font-[var(--font-cinzel)] text-sm tracking-[0.15em] uppercase hover:bg-[var(--color-gold)]/10 hover:text-[var(--color-cream)] transition-colors"
                    >
